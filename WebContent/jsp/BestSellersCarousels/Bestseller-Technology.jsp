...
<%
  DataSource ds_tec = (DataSource)application.getAttribute("DataSource");
  List<Books> all_tec = new BooksDao(ds_tec).findByCategory("14");
  // prendo fino a 12, ma mostro 5 per slide
  List<Books> books_tec = (all_tec.size() > 12) ? all_tec.subList(0, 12) : all_tec;
  final int perSlide_tec = 5;
%>

<div class="container my-4 position-relative" id="bestseller-carousel">
  <div class="d-flex justify-content-between align-items-baseline mb-3">
    <h2>Bestsellers Technology</h2>
    <a href="<%=request.getContextPath()%>/narrative">See All</a>
  </div>

  <div id="multiCarouselTechnology" class="carousel slide">
    <!-- Prev/Next visibili -->
    <button class="btn btn-outline-primary carousel-control-button prev"
            type="button" data-bs-target="#multiCarouselTechnology" data-bs-slide="prev">
      ‹ Prev
    </button>
    <button class="btn btn-outline-primary carousel-control-button next"
            type="button" data-bs-target="#multiCarouselTechnology" data-bs-slide="next">
      Next ›
    </button>

    <div class="carousel-inner">
      <% if (books_tec != null && !books_tec.isEmpty()) {
           for (int i = 0; i < books_tec.size(); i += perSlide_tec) {
             String active = (i == 0) ? " active" : "";
      %>
      <div class="carousel-item<%=active%>">
        <div class="d-flex">
          <% for (int j = 0; j < perSlide_tec; j++) {
               int idx = i + j;
               if (idx < books_tec.size()) {
