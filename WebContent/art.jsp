...
<% 
  ...
  @SuppressWarnings("unchecked")
  List<Books> artBooks = (List<Books>) session.getAttribute("artBooks");
  if (artBooks == null) {
    Context initCtx = new InitialContext();
    Context envCtx  = (Context) initCtx.lookup("java:comp/env");
    DataSource ds   = (DataSource) envCtx.lookup("jdbc/Bookstore");
    BooksDao dao    = new BooksDao(ds);
    artBooks     = dao.findByCategory("3");
    session.setAttribute("artBooks", artBooks);
  }
%>
...
<h1>All Arts Books</h1>

<div class="books-grid">
  <%
    if (artBooks != null && !artBooks.isEmpty()) {
      for (Books b : artBooks) {
  %>
    <div class="book-card">
